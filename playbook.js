...
export const CARD = {
  ...
  /* ===== הפוסטר השבועי ===== */
  /* הצילום שברצועה. יושב במאגר ולא בספרייה, בדיוק מאותה סיבה כמו
     הלוגו: פוסטר יוצא איתו מהרגע הראשון, גם לפני שמישהו ייבא מהדרייב.
     צילום מהספרייה שנבחר בבורר גובר עליו. */
  posterFile: "./poster.jpg",
  /* המשפט שעל הצילום. "נוף שאין לו סוף" היה סופרלטיב שיווקי, ובדיוק
     מה ש-VOICE אומר לא לעשות — וגם לא מה שרואים בצילום. שתי עובדות
     קצרות עובדות טוב יותר, וניתן לשנות מהמסך. */
  posterTag: "כוס קפה, וזמן שלא בוער",
  posterFocus: .42,                          // מיקוד החיתוך של הרצועה
  posterTime: "08:00",                      // שעת השיגור היומית
  posterAuto: true,                         // שיגור וסנכרון בלי ללחוץ
