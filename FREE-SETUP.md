...
7. באפליקציה, בלשונית **משמרות** ← כרטיס "שעות הפתיחה, בכל מקום" ← **חיבור עמוד פייסבוק (פעם אחת)** ← מדביקים את הטוקן ← **חבר עמוד**.
   השרת מחליף אותו בטוקן עמוד **ארוך טווח** ושומר אותו אצלו (Firestore, `secrets/meta`, סגור לקריאה). זהו — הטוקן של העמוד לא פג.
   אם יש כמה עמודים בחשבון, האפליקציה שואלת איזה מהם הוא העגלה.

זה דורש שהסוד `FIREBASE_SA` יהיה ב-Cloudflare (ראה `worker/README.md`). בלעדיו האפליקציה מציגה את
`FB_PAGE_ID` / `FB_PAGE_TOKEN` / `IG_USER_ID` להעתקה ידנית ל-Cloudflare — ומה שמוגדר שם תמיד מנצח.
